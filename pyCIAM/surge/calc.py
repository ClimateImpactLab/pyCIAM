...
def _calc_storm_capital_and_mortality(
    ...
    # return depth_st, this_exposure_dens, dmf_kwargs["vsl"]
    storm_capital = (
...
def calc_storm_damages_no_resilience(
    esl,
    exposure,
    elev_bounds,
    R,
    ...
    if stack and "vsl" in dmf_kwargs and isinstance(dmf_kwargs["vsl"], xr.DataArray):
        dmf_kwargs["vsl"] = dmf_kwargs["vsl"].where(valid)
        dmf_kwargs["vsl"] = (
            dmf_kwargs["vsl"]
            .stack(stacked=esl.stacked.to_index().names)
            .sel(stacked=esl.stacked)
        )
